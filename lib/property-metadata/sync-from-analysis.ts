...
function extractCandidates(
  ...
  const bedroomsRaw = r.bedrooms
  const bedrooms =
    typeof bedroomsRaw === "number" && Number.isFinite(bedroomsRaw) && bedroomsRaw >= 0 && bedroomsRaw <= 50
      ? Math.round(bedroomsRaw)
      : null
 ...
 * filled) values are NEVER overwritten.
 *
...
export async function syncPropertyMetadataFromAnalysis(
  ...
  const { data: existing, error: fetchErr } = await supabase
    .from("properties")
    .select("construction_year, heating_type, property_type, living_area_m2, bedrooms")
    .eq("id", propertyId)
    .maybeSingle()
  ...
  if (existingTyped.bedrooms === null && candidate.bedrooms !== null) {
    patch.bedrooms = candidate.bedrooms
  }
