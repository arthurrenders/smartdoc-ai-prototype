...
export async function getMapMarkers(): Promise<{
  ...
  try {
    ...
    const { data: rows, error: qErr } = await supabase
      ...
      .select("property_id, latitude, longitude, raw_line1, normalized_full_address")
    ...
    const markers: MapMarkerRow[] = base.map((b) => ({
      ...b,
